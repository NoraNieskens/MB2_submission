...
S2_forestmask1<-S2_forestmask/10000
...
```{r}
uc <- unsuperClass(S2_forestmask1, nClasses = 5)
ggR(uc$map, forceCat=T, geom_raster=T)
```

#extract environmental information from obs. location
```{r}
env <- raster::extract(S2_forestmask1, tree_sp, fun=mean, sp=F)
...
rf.grid <- expand.grid(mtry=1:24) # number of variables available for splitting at each tree node
...
rf_prediction = raster::predict(stack(S2_AOI), model=rf_model)
plot(rf_prediction)
...
#train random forest model 
set.seed(2) #make it reproducible
rf_model2 <- caret::train(x = tree_env[,names(tree_env)!=c("NMDS1","NMDS2","n_species")], y =tree_env$NMDS2,
                    method = "rf", metric="Rsquared", trainControl = tc, 
                    tuneGrid = rf.grid)

rf_model$results
max(rf_model2$results$Rsquared)
rf_model2$bestTune
...
# Apply the random forest model to the Sentinel-2 data
rf_prediction2 = raster::predict(stack(S2_AOI), model=rf_model2)
plot(rf_prediction2)
...
##Not including NMDS
#train random forest model   
set.seed(3) #make it reproducible
rf_model3 <- caret::train(x = tree_env[,names(tree_env)!=c("n_species","NMDS1", "NMDS2")], y =tree_env$n_species,
                    method = "rf", metric="Rsquared", trainControl = tc, 
                    tuneGrid = rf.grid)
...
# Apply the random forest model to the Sentinel-2 data
rf_prediction3 = raster::predict(stack(S2_AOI), model=rf_model3)
plot(rf_prediction3) 
...
rf_model4 <- caret::train(x = tree_env[,names(tree_env)!=c("n_species", "NMDS2")], y =tree_env$n_species,
                    method = "rf", metric="Rsquared", trainControl = tc, 
                    tuneGrid = rf.grid)
...
# Apply the random forest model to the Sentinel-2 data
rf_prediction4= raster::predict(stack(S2_AOI), model=rf_model4)
plot(rf_prediction4) 

```

The three parameters (NMDS1, NMDS2 and number of tree species) are predicted based on multi-temporal S2 imagery using Random forest. The results differ highly in model accuracy (NMDS1 R2= 0.84, NMDS2 R2 =0.21, n species R2 = 0.89  including NMDS1+2, n species R2 = 0.89  only S2 bands). Species richness has shown to be highly negatively correlated to NMDS1(R=-0.92), whereas there is no correlation to NMDS2 (0.07). 
